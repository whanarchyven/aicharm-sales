    ...
    if(body){
        return (
            <div className={'flex w-full items-end justify-between sm:text-lg text-xs gap-6'}>
                <p className={classList('sm:w-[80%] whitespace-pre-wrap font-light',color=='black'?'text-black':'text-white')}>{isOpen?body:body?.split(' ').splice(0,limiter).join(' ').concat(ending)}</p>
