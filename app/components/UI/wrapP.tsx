...
const WrapP = ({body,limiter,ending,color}:wrapPInterface) => {
    ...
    return (
        <div className={'flex items-end justify-between gap-6'}>
            <p className={classList('w-[90%] whitespace-pre-wrap font-extralight',color=='black'?'text-black':'text-white')}>{isOpen?body:body.split(' ').splice(0,limiter).join(' ').concat(ending)}</p>
            <div onClick={()=>{setIsOpen(!isOpen)}} className={'w-[10%] flex gap-3 cursor-pointer items-center'}>
